...
const layoutModeLabels: Record<NonNullable<Announcement["layoutMode"]>, { label: string; desc: string }> = {
    ...
    hero: { label: "رئيسي", desc: "حضور أقوى للوسيط والعنوان عند إطلاق حملة رئيسية." },
    split: { label: "مقسّم", desc: "تقسيم واضح بين الوسيط والمحتوى للحملات المركزة." },
    compact: { label: "مختصر", desc: "صيغة خفيفة للرسائل السريعة والتنبيهات المختصرة." },
};
...
export function AnnouncementsClient({
    ...
    const operationsSnapshot = useMemo(() => {
        ...
        if (routeTargets.length === 0) {
            recommendations.push("لا توجد حملات مرتبطة بصفحات بعينها. استخدم دخول الصفحة لرسائل الدفع أو التصميم أو الانضمام.");
        }
        if (liveNow.every((announcement) => announcement.trigger.frequency === "always") && liveNow.length > 0) {
            recommendations.push("الطابور الحي يعتمد بالكامل على الظهور في كل مرة. امزج معه ظهور الجلسة أو الظهور لمرة واحدة حتى لا يرهق الزائر.");
        }
            ...
            <section className="theme-surface-panel relative overflow-hidden rounded-[28px] p-5 sm:p-6 md:p-7">
                ...
                <div className="relative grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
                    <div className="space-y-5">
                        ...
                        <div className="space-y-3">
                            <p className="text-xs font-semibold text-theme-faint">قوالب تشغيل سريعة</p>
                                                ...
                                                <div className="flex items-center justify-between gap-3">
                                                    <div>
                                                        ...
                                                        <p className="mt-1 text-[11px] leading-6 text-theme-subtle">
                                                            تظهر قبل التشغيل وتفيد مع القوالب السينمائية والرئيسية.
                                                        </p>
                                                    </div>
