...
import { Image as ImageIcon, Type, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react';
    ...
    >
      ...
      <div className="space-y-8">
        ...
        <motion.div
          ...
          className="flex gap-3 p-1.5 bg-washa-bg/60 rounded-xl border border-washa-border/50 w-fit mx-auto backdrop-blur-sm"
        >
          <button
            onClick={() => updateState({ designMethod: 'text' })}
            className={cn(
              'px-6 py-3 rounded-lg text-sm font-medium transition-all duration-500 flex items-center gap-2',
          ...
          >
            <Type className="w-4 h-4" /> وصف نصي
          </button>
          <button
            onClick={() => updateState({ designMethod: 'image' })}
            className={cn(
              'px-6 py-3 rounded-lg text-sm font-medium transition-all duration-500 flex items-center gap-2',
          ...
          >
            <ImageIcon className="w-4 h-4" /> صورة مرجعية
          </button>
        </motion.div>
